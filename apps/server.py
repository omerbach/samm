...
def SupportRequest():
    ...
    mail.MailGunMail().send(to=['omerbach@gmail.com'],
                            ...
                            subject=('בקשת תמיכה : #%s, מלקוח : %s' % (
                            supportRequestId, utils.config.companyName.encode('utf-8'))).decode('utf-8'),
                            message=msg,
                            html=False,
                            )
    ...
    if root:
        for element in os.listdir(root):
            if os.path.isdir(os.path.join(root, element)):
                try:
                    sms_content = file(os.path.join(root, element, 'sms.htm')).read().decode(
                        'utf-16') if 'sms.htm' in os.listdir(os.path.join(root, element)) else ""
                    mail_content = file(os.path.join(root, element, 'mail.htm')).read().decode(
                        'utf-16') if 'mail.htm' in os.listdir(os.path.join(root, element)) else ""
                    letter_content = file(os.path.join(root, element, 'letter.htm')).read().decode(
                        'utf-16') if 'letter.htm' in os.listdir(os.path.join(root, element)) else ""
                    ...
                    folders.append({'name': element,
                                    ...
                                    'letter': True if 'letter.htm' in os.listdir(
                                        os.path.join(root, element)) or 'mail.htm' in os.listdir(
                                        os.path.join(root, element)) else False,
...
def GetMultiTypeElements(include_specific_buildings=False, tenants_from_building_id=0):
    ...
    for entity_type in ['building', 'professional', 'worker'] if not tenants_from_building_id else ['professional',
                                                                                                    'worker']:
        cursor.execute('''
            SELECT %s_id, name
            FROM %ss        
...
def fetchProfessionals():
    lastUpdatedProfessional = bottle.request.query.updated_professional if len(
        bottle.request.query.updated_professional) else 0
...
def fetchDistinctPreventionNames():
    ...
    with info_db as connection:
        cursor = connection.cursor()
        cursor.execute(
            'select distinct description, category, professional_id from prevention order by description asc')
...
def fetchPreventions():
    ...
    lastUpdatedPrevention = bottle.request.query.updated_prevention_id if len(
        bottle.request.query.updated_prevention_id) else 0
        ...
        # insert new
            ...
            for i in range(quantity):
                ...
                ''', (payment_details.get("status", ""),
                      ...
                      payment_details.get("payment_approval", ""),
                      '%s' % (dates.date(tenant_cheque_date) + relativedelta(months=i)) if len(
                          tenant_cheque_date) else "",
                      payment_details.get("tenant_bank_account", ""),
                      payment_details.get("tenant_bank_branch", ""),
                      payment_details.get("company_bank_account", ""),
                      payment_details.get("company_bank_branch", ""),
...
def fetchPayments():
    ...
    start_date = bottle.request.query.start_date
    end_date = bottle.request.query.end_date
    lastUpdatedPaymentRequests = json.loads(bottle.request.query.updated_payment_ids) if len(
        bottle.request.query.updated_payment_ids) else []
    ...
    cursor.execute('SELECT COUNT(*) FROM payments as p WHERE DATE(p.tenant_cheque_date) between ? AND ? %s' % query_ext,
                   parameters)
    res = cursor.fetchone()
    if res:
        maximum_records, = res
    ...
    templates = [
        {
            ...
            'name': tName,
            'comment': tComment,
            'sms_content': open(os.path.join('templates', tName, 'sms.htm')).read() if os.path.exists(
                os.path.join('templates', tName, 'sms.htm')) else "",
            'mail_content': open(os.path.join('templates', tName, 'mail.htm')).read() if os.path.exists(
                os.path.join('templates', tName, 'mail.htm')) else "",
            'mail_subject': open(os.path.join('templates', tName, 'mailSubject.htm')).read() if os.path.exists(
                os.path.join('templates', tName, 'mailSubject.htm')) else "",
            'letter_content': open(os.path.join('templates', tName, 'letter.htm')).read() if os.path.exists(
                os.path.join('templates', tName, 'letter.htm')) else "",
...
def fetchServiceRequests():
    ...
    start_date = bottle.request.query.start_date
    end_date = bottle.request.query.end_date
    lastUpdatedServiceRequest = bottle.request.query.updated_service_request_id if len(
        bottle.request.query.updated_service_request_id) else 0
    ...
    cursor.execute('SELECT COUNT(*) FROM service as s WHERE DATE(s.start_date) between ? AND ? %s' % query_ext,
                   parameters)
    res = cursor.fetchone()
    if res:
        maximum_records, = res
    ...
    serviceOf = {sId: {
        ...
        'must_attend': (dates.date(sStartDate) + datetime.timedelta(
            days=utils.config.service_sla)) < datetime.datetime.now().date() and (
                                   int(sStatus) == 1 or int(sStatus) == 2),
        'building_name': building_name,
        'tenant_name': tenant_name,
        'worker_name': worker_name,
        'professional_name': professional_name,
...
def guessProfessional():
    ...
    cursor.execute(query)
    guesses = list(
        {'id': professionalId, 'name': professionalName} for professionalId, professionalName in cursor.fetchall())
    ...
    alerts = [
        {
            ...
            'external_folder': external_folder,
            'alert_data': open(alertPathToFile).read() if os.path.exists(
                alertPathToFile) else "%s does not exist" % alertPathToFile,
            'alert_selected': False
        }
        for
        aId, rType, rName, bName, alertPathToFile, aType, alertMetaData, aDestination, aSource, alertTimeStamp, external_folder
        in cursor]
...
def resendAlertsFromHistory():
    ...
    with info_db as connection:
        ...
        for recepientType, recepientName, buildingName, pathToFile, alertType, alertMeta, alertDestination, source in cursor.fetchall():
            ...
            sendAlert(alertData, alertMeta, alertType, alertDestination, buildingName, recepientName, recepientType,
                      source)
	...
	WHERE service_id in (%s)
        ...
        for service_id, service_description, service_status, building_name, professional_name, professional_phones, tenant_name, worker_name, tenant_phones, professional_phones in cursor.fetchall():
            ...
            for phone in professional_phones.split(', '):
                sendAlert(content, SmsCreditMessage(content), 0, phone, building_name, professional_name, 1,
                          source_phone)
...
def exportHtmlTableToPdf():
    ...
    content = alerters.Alerter.GetTemplateContent('web/customer_templates/service_requests.html', None,
...
def getServiceReminderData():
    ...
    service_data = resolveAlertTemplate(params.path, params.alert, json.loads(params.details), file(
        os.path.join(params.path, params.alert + '.htm').replace('\\', '/')).read())
...
def storeAlertToHistory(alertContent, alertMetaData, alertType, alertDestination, buildingName, recepientName,
    ...
    with info_db as connection:
        ...
        ''', (recepientName, buildingName, recepientType, tmpFile, alertType, alertMetaData, alertDestination, source,
              external_folder))
...
def do_upload():
    ...
    tmpFolder = utils.CreateTempFolder(dir='attachments') if not len(uploaded_folder) else os.path.join('attachments',
                                                                                                        uploaded_folder)
...
def sendTemplateAlert():
    ...
    for destination in destinations:
        sendAlert(fileContent, metaData, alertsTypeOf[alert], destination, dest["building"], dest["name"],
                  recepient_type, source)
