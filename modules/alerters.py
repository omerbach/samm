...
class Alerter(object):
        ...
        return Alerter.GetTemplateContent('web/templates/reports/executive_report.html', None,
                                          {
                                              ...
                                              'months': utils.HebrewMonths(','.join(
                                                  [str(month) for month in requestParameters.reportingMonths])),
                                              'total': total,
    ...
    def PrintingDialog(billboardsOf, lettersOf):
        ...
        if letters2:
            newLetters2 = ['<hr class="no_print">%s<span class="page-break"></span>' % letter for letter in
                           letters2[:-1]]
        ...
        alertTemplateContent = Alerter.GetTemplateContent(path, templateStr,
                                                          {
                                                              ...
                                                              'company_fax': utils.config.companyFax,
                                                              'company_address': utils.config.companyAddress,
                                                              'previous_month': '%02d' % 12 if int(
                                                                  datetime.datetime.now().month) == 1 else int(
                                                                  datetime.datetime.now().month) - 1,
                                                              'current_month': '%02d' % int(
                                                                  datetime.datetime.now().month),
                                                              'next_month': '%02d' % 1 if int(
                                                                  datetime.datetime.now().month) == 12 else int(
                                                                  datetime.datetime.now().month) + 1,
    ...
    def FormatBillboardTemplate(customerTemplatesDir,
        ...
        debtsTenantsTable = Alerter.GetTemplateContent(debtsTenantsTableTemplate, None,
                                                       {'tenants': tenants}) if os.path.exists(
            debtsTenantsTableTemplate) else None
        paymentsTenantsTable = Alerter.GetTemplateContent(paymentsTenantsTableTemplate, None,
                                                          {'tenants': tenants}) if os.path.exists(
            paymentsTenantsTableTemplate) else None
        generalTenantsTable = Alerter.GetTemplateContent(generalTenantsTableTemplate, None,
                                                         {'tenants': tenants}) if os.path.exists(
            generalTenantsTableTemplate) else None
    ...
    @staticmethod
    def DumpTenantAlert(executionType, customerTemplatesDir, msg, building, appartment, name, alertDestination, alert,
                        description):
